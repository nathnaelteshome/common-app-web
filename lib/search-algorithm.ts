...
export class SearchIndex<T extends SearchableItem> {
  ...
  build(items: T[]): void {
    ...
    console.log(
      `Indexed ${items.length} items with ${this.tokenIndex.size} unique tokens`
    );
  }
  ...
  search(query: string, options: SearchOptions = {}): SearchResult<T>[] {
    ...
    console.timeEnd("Search Execution");
    console.log(
      `Found ${results.length} results, returning top ${Math.min(
        maxResults,
        results.length
      )}`
    );
  ...
  private scoreItem(
    ...
    const fields = [
      {
        name: "name",
        text: normalizedItem.normalizedName,
        boost: boostFactors.name!,
      },
      {
        name: "type",
        text: normalizedItem.normalizedType,
        boost: boostFactors.type!,
      },
      {
        name: "location",
        text: normalizedItem.normalizedLocation,
        boost: boostFactors.location!,
      },
      {
        name: "description",
        text: normalizedItem.normalizedDescription,
        boost: boostFactors.description!,
      },
    ];
